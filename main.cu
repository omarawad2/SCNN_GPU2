...
//naive implementation
__global__ void kCount_effectual_activations(int n, int channel, int sx, int sy, int X, int Y, int stride,
        const float* d_act, uint64_t &d_act_queue_count, float* d_act_queue, int* d_act_queue_x, int* d_act_queue_y,
        bool populate){
    ...
    if(x < X){
        ...
        if(y < Y){
            int tmp_sy = y % stride;
            float act_bits = 0;//layer.act_get(n,channel,x,y);
...
//naive implementation
__global__ void kCount_effectual_weights(int ck, int sx, int sy, int R, int S, int k_end, int stride, int padding,
        const float* d_wgt, uint64_t &d_wgt_queue_count, float* d_wgt_queue, int* d_wgt_queue_k, int* d_wgt_queue_r,
        int* d_wgt_queue_s, bool populate){
    ...
    if(r < R){
        ...
        if(s < S){
            ...
            if(k < k_end){
                float wgt_bits = 0;//layer.wgt_get(k,ck,r,s);
...
void count_effectual_activations(int n, int channel, int sx, int sy, int X, int Y, int stride, const Layer &layer,
        uint64_t &d_act_queue_count, float* d_act_queue, int* d_act_queue_x, int* d_act_queue_y, bool populate){

    auto bytes = layer.getMaxIndex("activations") * sizeof(float);
    float* d_act = host2Dev(bytes, layer.activations,"allocate device activations channel");

    dim3 block(32, 32);
    dim3 grid((X+block.x-1)/block.x,(Y+block.y-1)/block.y);
    //TODO:add streams
    kCount_effectual_activations<<<grid,block>>>(n,channel,sx,sy,X,Y,stride,d_act,d_act_queue_count,d_act_queue,
            d_act_queue_x,d_act_queue_y,populate);
    cudaDeviceSynchronize();
...
void count_effectual_weights(int ck, int sx, int sy, int R, int S, int k_end, int stride, int padding,
        const Layer &layer, uint64_t &d_wgt_queue_count, float* d_wgt_queue, int* d_wgt_queue_k, int* d_wgt_queue_r,
        int* d_wgt_queue_s, bool populate){

    auto bytes = layer.getMaxIndex("weights") * sizeof(float);
    float* d_wgt = host2Dev(bytes, layer.activations,"allocate device weights channel");

    dim3 block(16, 16, 4);
    dim3 grid((R+block.x-1)/block.x,(S+block.y-1)/block.y,(k_end+block.z-1)/block.z);
    //TODO:add streams
    kCount_effectual_weights<<<grid,block>>>(ck,sx,sy,R,S,k_end,stride,padding,d_wgt,d_wgt_queue_count,d_wgt_queue,
            d_wgt_queue_k,d_wgt_queue_r,d_wgt_queue_s,populate);
    cudaDeviceSynchronize();
...
ck: weights channel
kc: starting weight (Two towers AlexNet)
...
void computeTile(int n, int ct, int ck, int kc, int Kc, int X, int Y, int K, int W, int H, int R, int S,
    ...
    for(int sx = 0; sx < stride; sx++){
        for(int sy = 0; sy < stride; sy++){
            ...
            count_effectual_activations(n,ct+ck,sx,sy,X,Y,stride,layer,act_queue_count,d_act_queue,d_act_queue_x,
                    d_act_queue_y,false);

            // Count number of effectual weights
            uint64_t wgt_queue_count = 0;
            count_effectual_weights(ck,sx,sy,R,S,kc+Kc,stride,padding,layer,wgt_queue_count,d_wgt_queue,
                    d_wgt_queue_k,d_wgt_queue_r,d_wgt_queue_s,false);
            ...
            count_effectual_weights(ck,sx,sy,R,S,kc+Kc,stride,padding,layer,wgt_queue_count,d_wgt_queue,
                    d_wgt_queue_k,d_wgt_queue_r,d_wgt_queue_s,true);
